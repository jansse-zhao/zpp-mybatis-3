...
public class Configuration {
	...
	/**
	 * @param transaction:  事务实例对象
	 * @param executorType: 执行器类型simple、reuse、batch；configuration中配置的默认的执行器类型是simple
	 * @Author zpp
	 * @Description 根据传入的执行器类型创建对应的执行器实例对象
	 * @Date 2023/12/15 9:38
	 * @Return org.apache.ibatis.executor.Executor
	 */
	public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
		...
		// 将执行器注册到系统中配置的拦截器链中，要想在业务中拦截执行器，
		// 需要在拦截器上配置@Interceptors({@Signature(type=Executor.class, method="update", args={String.class})})
	...
	/**
	 * 配置拦截器，该方法在XMLConfigBuilder中解析xml的<plugins>标签时调用
	 */
