...
public class XMLConfigBuilder extends BaseBuilder {
	...
	/**
	 * @Description 将reader通过XPathParser在包装一下
	 * 1. org.apache.ibatis.session.SqlSessionFactoryBuilder.build<51></>处调用
	 */
	...
	/**
	 * @Description 将inputStream通过XPathParser在包装一下
	 * 1. org.apache.ibatis.session.SqlSessionFactoryBuilder.build<81></>处调用
	 */
	...
	/**
	 * XMLConfigBuilder实例对象构造完成后，它就有了
	 * 1. Configuration配置类对象
	 * 2. parser xml解析器对象，专门解析mybatis-config.xml
	 * 3. parsed 是否解析过mybatis-config.xml
	...
	private XMLConfigBuilder(Class<? extends Configuration> configClass, XPathParser parser, String environment,
		...
		// 给configuration对象设置用户设置的变量对象props，可能为null
	...
	public Configuration parse() {
		...
		parsed = true;
		// 解析mybatis配置文件根<configuration></configuration>
		// 这一步会将mybatis.xml配置文件中的所有属性都解析到configuration对象中
	...
	private void parseConfiguration(XNode root) {
		try {
			...
			// 解析配置文件中properties属性下的值
			propertiesElement(root.evalNode("properties"));
			// 解析配置文件中settings属性下的值，并校验是否有用户自定义的setting属性
			Properties settings = settingsAsProperties(root.evalNode("settings"));
			// setting中是否配置有vfsImpl实现，有则设置到Configuration中
			loadCustomVfsImpl(settings);
			// setting中是否配置有logImpl实现，有则设置到Configuration中
	...
	private Properties settingsAsProperties(XNode context) {
		...
		// 通过localReflectorFactory获取一个Configuration类的反射器工具，在通过MetaClass对象调用反射器工具获取Configuration类的对应信息
		MetaClass metaConfig = MetaClass.forClass(Configuration.class, localReflectorFactory);
		// 校验所有的setting属性是否为mybatis指定的，自己加是setting不中
